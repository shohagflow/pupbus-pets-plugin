...
function pupbus_pets_portal_shortcode(): string
{
	...
	<section class="pupbus-pets-wrapper pupbus-pets-portal-section">
		...
		<?php else : ?>
			<?php echo wp_kses_post( pupbus_pets_auth_form_markup( ) ); ?>
		<?php endif; ?>
	</section>
	<?php
	return (string ) ob_get_clean();
...
function pupbus_pets_render_applications_page(): void
{
	...
	?>
	<div class="wrap">
		<h1><?php esc_html_e( 'Submitted Applications', 'pupbus-pets' ); ?></h1>
		<p><?php esc_html_e( 'Feature coming soon: View and manage all submitted pup applications here.', 'pupbus-pets' ); ?></p>
	</div>
	<?php
}
...
function pupbus_pets_render_admin_page(): void
{
	...
	if (isset( $_POST['pupbus_pets_save_settings'] )) {
		check_admin_referer( 'pupbus_pets_settings_action', 'pupbus_pets_settings_nonce' );
		$redirect_page_id = isset( $_POST['pupbus_pets_redirect_page'] ) ? absint( $_POST['pupbus_pets_redirect_page'] ) : 0;
		update_option( 'pupbus_pets_redirect_page_id', $redirect_page_id );
		echo '<div class="notice notice-success is-dismissible"><p>' . esc_html__( 'Settings saved.', 'pupbus-pets' ) . '</p></div>';
	}

	$shortcode = '[pupbus_pets_portal]';
	$selected_page_id = get_option( 'pupbus_pets_redirect_page_id', 0 );
	?>
	<div class="wrap">
		...
		<table class="form-table" role="presentation">
			<tbody>
				...
				<tr>
					<th scope="row"><label for="pupbus-pets-apply-shortcode"><?php esc_html_e( 'Apply Now Shortcode', 'pupbus-pets' ); ?></label></th>
					<td>
						<input
							id="pupbus-pets-apply-shortcode"
							type="text"
							class="regular-text code"
							readonly
							value="[pupbus_pets_apply_now]"
						/>
						<button type="button" class="button button-primary" id="pupbus-pets-apply-copy-btn"><?php esc_html_e( 'Copy Shortcode', 'pupbus-pets' ); ?></button>
						<p class="description" id="pupbus-pets-apply-copy-status"><?php esc_html_e( 'Ready to copy.', 'pupbus-pets' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><label for="pupbus-pets-success-shortcode"><?php esc_html_e( 'Application Success Shortcode', 'pupbus-pets' ); ?></label></th>
					<td>
						<input
							id="pupbus-pets-success-shortcode"
							type="text"
							class="regular-text code"
							readonly
							value="[pupbus_pets_apply_success]"
						/>
						<button type="button" class="button button-primary" id="pupbus-pets-success-copy-btn"><?php esc_html_e( 'Copy Shortcode', 'pupbus-pets' ); ?></button>
						<p class="description" id="pupbus-pets-success-copy-status"><?php esc_html_e( 'Ready to copy.', 'pupbus-pets' ); ?></p>
					</td>
				</tr>
			</tbody>
		</table>
		...
		<h2><?php esc_html_e( 'Application Settings', 'pupbus-pets' ); ?></h2>
		<form method="post">
			<?php wp_nonce_field( 'pupbus_pets_settings_action', 'pupbus_pets_settings_nonce' ); ?>
			<table class="form-table" role="presentation">
				<tbody>
					<tr>
						<th scope="row"><label for="pupbus-pets-redirect-page"><?php esc_html_e( 'Redirect After Submission', 'pupbus-pets' ); ?></label></th>
						<td>
							<?php
							wp_dropdown_pages(
								array(
									'name'			  => 'pupbus_pets_redirect_page', 
									'id'				=> 'pupbus-pets-redirect-page', 
									'show_option_none'  => __( 'Select a page', 'pupbus-pets' ), 
									'option_none_value' => '0', 
									'selected'		  => $selected_page_id, 
								)
							);
							?>
							<p class="description"><?php esc_html_e( 'Select the page where users should be redirected after submitting the Apply Now form (e.g., your Application Success page ).', 'pupbus-pets' ); ?></p>
						</td>
					</tr>
				</tbody>
			</table>
			<p class="submit">
				<input type="submit" name="pupbus_pets_save_settings" class="button button-primary" value="<?php esc_attr_e( 'Save Application Settings', 'pupbus-pets' ); ?>" />
			</p>
		</form>
	</div>
	...
	<script>
		(function () {
			function setupCopy( btnId, fieldId, statusId ) {
				var copyButton = document.getElementById( btnId );
				var shortcodeField = document.getElementById( fieldId );
			...
			setupCopy( 'pupbus-pets-copy-btn', 'pupbus-pets-shortcode', 'pupbus-pets-copy-status' );
			setupCopy( 'pupbus-pets-apply-copy-btn', 'pupbus-pets-apply-shortcode', 'pupbus-pets-apply-copy-status' );
			setupCopy( 'pupbus-pets-success-copy-btn', 'pupbus-pets-success-shortcode', 'pupbus-pets-success-copy-status' );
		} )();
	</script>
	<?php
}
