...
/**
 * Render the Apply Now form.
 */
function pupbus_pets_apply_now_markup(): string
{
	ob_start();
	?>
...
function pupbus_pets_handle_apply_now(): void
{
	...
	// In a real scenario, you'd send an email or save to DB.
	...
	// Redirect based on admin settings
	$redirect_page_id = get_option( 'pupbus_pets_redirect_page_id', 0 );
	$redirect_url = home_url( '/' );

	if ($redirect_page_id ) {
		$permalink = get_permalink( $redirect_page_id );
		if ($permalink ) {
...
function pupbus_pets_apply_now_shortcode(): string
{
	...
	return pupbus_pets_apply_now_markup();
}
add_shortcode( 'pupbus_pets_apply_now', 'pupbus_pets_apply_now_shortcode' );

/**
 * Render the success page after submission.
 */
function pupbus_pets_apply_success_markup(): string
{
	ob_start();
	?>
	<div class="pupbus-pets-apply-shell">
...
/**
 * Shortcode for Application Success page.
 */
function pupbus_pets_apply_success_shortcode(): string
{
	pupbus_pets_enqueue_assets();
	return pupbus_pets_apply_success_markup();
}
